...
#ifndef INITIALIZER_EXOTICA_QUADROTOR_DYNAMICS_SOLVER_QUADROTOR_DYNAMICS_SOLVER_INITIALIZER_H
#define INITIALIZER_EXOTICA_QUADROTOR_DYNAMICS_SOLVER_QUADROTOR_DYNAMICS_SOLVER_INITIALIZER_H

#include <exotica_core/property.h>

namespace exotica
{
inline std::vector<Initializer> GetExoticaQuadrotorDynamicsSolverInitializers();
}

namespace exotica
{

class QuadrotorDynamicsSolverInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/QuadrotorDynamicsSolver" ;}

    QuadrotorDynamicsSolverInitializer() : InitializerBase(),
        Debug( false) ,
        dt( 0.01) ,
        Integrator( "RK1") ,
    ...
    QuadrotorDynamicsSolverInitializer( std::string _Name, bool _Debug =  false, double _dt =  0.01, std::string _Integrator =  "RK1", Eigen::VectorXd _ControlLimitsLow =  Eigen::VectorXd(), Eigen::VectorXd _ControlLimitsHigh =  Eigen::VectorXd()) : InitializerBase(),
        Name(_Name) ,
        Debug(_Debug) ,
        dt(_dt) ,
    ...
    QuadrotorDynamicsSolverInitializer(const Initializer& other) : QuadrotorDynamicsSolverInitializer()
    {
    ...
    {
        return (Initializer)QuadrotorDynamicsSolverInitializer();
    }

    virtual std::vector<Initializer> GetAllTemplates() const
    {
        return GetExoticaQuadrotorDynamicsSolverInitializers();
    }

    virtual void Check(const Initializer& other) const
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer QuadrotorDynamicsSolverInitializer requires property Name to be set!");
    }
...
#include<exotica_quadrotor_dynamics_solver/exotica_quadrotor_dynamics_solver_initializers_numerator.h>
