...
#ifndef INITIALIZER_EXOTICA_PINOCCHIO_DYNAMICS_SOLVER_PINOCCHIO_DYNAMICS_SOLVER_INITIALIZER_H
#define INITIALIZER_EXOTICA_PINOCCHIO_DYNAMICS_SOLVER_PINOCCHIO_DYNAMICS_SOLVER_INITIALIZER_H
...
class PinocchioDynamicsSolverInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/PinocchioDynamicsSolver" ;}

    PinocchioDynamicsSolverInitializer() : InitializerBase(),
        Debug( false) ,
        dt( 0.01) ,
        Integrator( "RK1") ,
    ...
    PinocchioDynamicsSolverInitializer( std::string _Name, bool _Debug =  false, double _dt =  0.01, std::string _Integrator =  "RK1", Eigen::VectorXd _ControlLimitsLow =  Eigen::VectorXd(), Eigen::VectorXd _ControlLimitsHigh =  Eigen::VectorXd()) : InitializerBase(),
        Name(_Name) ,
        Debug(_Debug) ,
        dt(_dt) ,
    ...
    PinocchioDynamicsSolverInitializer(const Initializer& other) : PinocchioDynamicsSolverInitializer()
    {
    ...
    {
        return (Initializer)PinocchioDynamicsSolverInitializer();
    }

    virtual std::vector<Initializer> GetAllTemplates() const
    {
        return GetExoticaPinocchioDynamicsSolverInitializers();
    }

    virtual void Check(const Initializer& other) const
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer PinocchioDynamicsSolverInitializer requires property Name to be set!");
    }
...
#include<exotica_pinocchio_dynamics_solver/exotica_pinocchio_dynamics_solver_initializers_numerator.h>
