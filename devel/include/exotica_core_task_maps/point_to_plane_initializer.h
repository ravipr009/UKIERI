...
#ifndef INITIALIZER_EXOTICA_CORE_TASK_MAPS_POINT_TO_PLANE_INITIALIZER_H
#define INITIALIZER_EXOTICA_CORE_TASK_MAPS_POINT_TO_PLANE_INITIALIZER_H
...
class PointToPlaneInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/PointToPlane" ;}

    PointToPlaneInitializer() : InitializerBase(),
        Debug( false) ,
        EndEffector( std::vector<exotica::Initializer>()) 
    {
    }

    PointToPlaneInitializer( std::string _Name, bool _Debug =  false, std::vector<exotica::Initializer> _EndEffector =  std::vector<exotica::Initializer>()) : InitializerBase(),
        Name(_Name) ,
        Debug(_Debug) ,
    ...
    PointToPlaneInitializer(const Initializer& other) : PointToPlaneInitializer()
    {
    ...
    {
        return (Initializer)PointToPlaneInitializer();
    }
    ...
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer PointToPlaneInitializer requires property Name to be set!");
    }
