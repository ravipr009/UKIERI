...
#ifndef INITIALIZER_EXOTICA_CORE_TASK_MAPS_EFF_AXIS_ALIGNMENT_INITIALIZER_H
#define INITIALIZER_EXOTICA_CORE_TASK_MAPS_EFF_AXIS_ALIGNMENT_INITIALIZER_H
...
class EffAxisAlignmentInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/EffAxisAlignment" ;}

    EffAxisAlignmentInitializer() : InitializerBase(),
        Debug( false) ,
        EndEffector( std::vector<exotica::Initializer>()) 
    {
    }

    EffAxisAlignmentInitializer( std::string _Name, bool _Debug =  false, std::vector<exotica::Initializer> _EndEffector =  std::vector<exotica::Initializer>()) : InitializerBase(),
        Name(_Name) ,
        Debug(_Debug) ,
    ...
    EffAxisAlignmentInitializer(const Initializer& other) : EffAxisAlignmentInitializer()
    {
    ...
    {
        return (Initializer)EffAxisAlignmentInitializer();
    }
    ...
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer EffAxisAlignmentInitializer requires property Name to be set!");
    }
