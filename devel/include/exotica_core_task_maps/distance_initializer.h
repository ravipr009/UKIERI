...
#ifndef INITIALIZER_EXOTICA_CORE_TASK_MAPS_DISTANCE_INITIALIZER_H
#define INITIALIZER_EXOTICA_CORE_TASK_MAPS_DISTANCE_INITIALIZER_H
...
class DistanceInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/Distance" ;}

    DistanceInitializer() : InitializerBase(),
        Debug( false) ,
        EndEffector( std::vector<exotica::Initializer>()) 
    {
    }

    DistanceInitializer( std::string _Name, bool _Debug =  false, std::vector<exotica::Initializer> _EndEffector =  std::vector<exotica::Initializer>()) : InitializerBase(),
        Name(_Name) ,
        Debug(_Debug) ,
    ...
    DistanceInitializer(const Initializer& other) : DistanceInitializer()
    {
    ...
    {
        return (Initializer)DistanceInitializer();
    }
    ...
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer DistanceInitializer requires property Name to be set!");
    }
