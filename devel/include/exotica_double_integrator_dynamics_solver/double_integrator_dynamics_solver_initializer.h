...
#ifndef INITIALIZER_EXOTICA_DOUBLE_INTEGRATOR_DYNAMICS_SOLVER_DOUBLE_INTEGRATOR_DYNAMICS_SOLVER_INITIALIZER_H
#define INITIALIZER_EXOTICA_DOUBLE_INTEGRATOR_DYNAMICS_SOLVER_DOUBLE_INTEGRATOR_DYNAMICS_SOLVER_INITIALIZER_H
...
class DoubleIntegratorDynamicsSolverInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/DoubleIntegratorDynamicsSolver" ;}

    DoubleIntegratorDynamicsSolverInitializer() : InitializerBase(),
        Debug( false) ,
        dt( 0.01) ,
        Integrator( "RK1") ,
    ...
    DoubleIntegratorDynamicsSolverInitializer( std::string _Name, bool _Debug =  false, double _dt =  0.01, std::string _Integrator =  "RK1", Eigen::VectorXd _ControlLimitsLow =  Eigen::VectorXd(), Eigen::VectorXd _ControlLimitsHigh =  Eigen::VectorXd()) : InitializerBase(),
        Name(_Name) ,
        Debug(_Debug) ,
        dt(_dt) ,
    ...
    DoubleIntegratorDynamicsSolverInitializer(const Initializer& other) : DoubleIntegratorDynamicsSolverInitializer()
    {
    ...
    {
        return (Initializer)DoubleIntegratorDynamicsSolverInitializer();
    }

    virtual std::vector<Initializer> GetAllTemplates() const
    {
        return GetExoticaDoubleIntegratorDynamicsSolverInitializers();
    }

    virtual void Check(const Initializer& other) const
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer DoubleIntegratorDynamicsSolverInitializer requires property Name to be set!");
    }
...
#include<exotica_double_integrator_dynamics_solver/exotica_double_integrator_dynamics_solver_initializers_numerator.h>
