...
#ifndef INITIALIZER_EXOTICA_CORE_TASK_MAP_INITIALIZER_H
#define INITIALIZER_EXOTICA_CORE_TASK_MAP_INITIALIZER_H
...
class TaskMapInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/TaskMap" ;}

    TaskMapInitializer() : InitializerBase(),
        Debug( false) ,
        EndEffector( std::vector<exotica::Initializer>()) 
    {
    }

    TaskMapInitializer( std::string _Name, bool _Debug =  false, std::vector<exotica::Initializer> _EndEffector =  std::vector<exotica::Initializer>()) : InitializerBase(),
        Name(_Name) ,
        Debug(_Debug) ,
    ...
    TaskMapInitializer(const Initializer& other) : TaskMapInitializer()
    {
    ...
    {
        return (Initializer)TaskMapInitializer();
    }
    ...
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer TaskMapInitializer requires property Name to be set!");
    }
