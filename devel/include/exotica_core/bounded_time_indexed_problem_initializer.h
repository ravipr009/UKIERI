...
#ifndef INITIALIZER_EXOTICA_CORE_BOUNDED_TIME_INDEXED_PROBLEM_INITIALIZER_H
#define INITIALIZER_EXOTICA_CORE_BOUNDED_TIME_INDEXED_PROBLEM_INITIALIZER_H
...
class BoundedTimeIndexedProblemInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/BoundedTimeIndexedProblem" ;}

    BoundedTimeIndexedProblemInitializer() : InitializerBase(),
        Debug( false) ,
    ...
    BoundedTimeIndexedProblemInitializer( std::string _Name, exotica::Initializer _PlanningScene, int _T, double _tau, bool _Debug =  false, std::vector<exotica::Initializer> _Maps =  std::vector<exotica::Initializer>(), Eigen::VectorXd _StartState =  Eigen::VectorXd(), double _StartTime =  0, int _DerivativeOrder =  -1, double _Wrate =  1.0, Eigen::VectorXd _W =  Eigen::VectorXd(), std::vector<exotica::Initializer> _Cost =  std::vector<exotica::Initializer>(), Eigen::VectorXd _LowerBound =  Eigen::VectorXd(), Eigen::VectorXd _UpperBound =  Eigen::VectorXd()) : InitializerBase(),
        Name(_Name) ,
    ...
    BoundedTimeIndexedProblemInitializer(const Initializer& other) : BoundedTimeIndexedProblemInitializer()
    {
    ...
    {
        return (Initializer)BoundedTimeIndexedProblemInitializer();
    }
    ...
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer BoundedTimeIndexedProblemInitializer requires property Name to be set!");
        if(!other.HasProperty("PlanningScene") || !other.properties_.at("PlanningScene").IsSet()) ThrowPretty("Initializer BoundedTimeIndexedProblemInitializer requires property PlanningScene to be set!");
        if(!other.HasProperty("T") || !other.properties_.at("T").IsSet()) ThrowPretty("Initializer BoundedTimeIndexedProblemInitializer requires property T to be set!");
        if(!other.HasProperty("tau") || !other.properties_.at("tau").IsSet()) ThrowPretty("Initializer BoundedTimeIndexedProblemInitializer requires property tau to be set!");
    }
