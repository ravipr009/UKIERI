...
#ifndef INITIALIZER_EXOTICA_CORE_UNCONSTRAINED_END_POSE_PROBLEM_INITIALIZER_H
#define INITIALIZER_EXOTICA_CORE_UNCONSTRAINED_END_POSE_PROBLEM_INITIALIZER_H
...
class UnconstrainedEndPoseProblemInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/UnconstrainedEndPoseProblem" ;}

    UnconstrainedEndPoseProblemInitializer() : InitializerBase(),
        Debug( false) ,
        ...
        W( Eigen::VectorXd()) ,
        NominalState( Eigen::VectorXd()) 
    {
    }

    UnconstrainedEndPoseProblemInitializer( std::string _Name, exotica::Initializer _PlanningScene, bool _Debug =  false, std::vector<exotica::Initializer> _Maps =  std::vector<exotica::Initializer>(), Eigen::VectorXd _StartState =  Eigen::VectorXd(), double _StartTime =  0, int _DerivativeOrder =  -1, std::vector<exotica::Initializer> _Cost =  std::vector<exotica::Initializer>(), Eigen::VectorXd _W =  Eigen::VectorXd(), Eigen::VectorXd _NominalState =  Eigen::VectorXd()) : InitializerBase(),
        Name(_Name) ,
    ...
    UnconstrainedEndPoseProblemInitializer(const Initializer& other) : UnconstrainedEndPoseProblemInitializer()
    {
        ...
        if (other.HasProperty("NominalState")) {const Property& prop=other.properties_.at("NominalState"); if(prop.IsSet()) NominalState = prop.IsStringType()?ParseVector<double,Eigen::Dynamic>(boost::any_cast<std::string>(prop.Get())):boost::any_cast<Eigen::VectorXd>(prop.Get());}
    }

    virtual Initializer GetTemplate() const
    {
        return (Initializer)UnconstrainedEndPoseProblemInitializer();
    }
    ...
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer UnconstrainedEndPoseProblemInitializer requires property Name to be set!");
        if(!other.HasProperty("PlanningScene") || !other.properties_.at("PlanningScene").IsSet()) ThrowPretty("Initializer UnconstrainedEndPoseProblemInitializer requires property PlanningScene to be set!");
    }
    ...
    operator Initializer()
    {
        ...
        ret.properties_.emplace("NominalState", Property("NominalState", false, boost::any(NominalState)));
        return ret;
    }
