...
#ifndef INITIALIZER_EXOTICA_OMPL_CONTROL_SOLVER_OMPL_CONTROL_SOLVER_INITIALIZER_H
#define INITIALIZER_EXOTICA_OMPL_CONTROL_SOLVER_OMPL_CONTROL_SOLVER_INITIALIZER_H
...
class OMPLControlSolverInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/OMPLControlSolver" ;}

    OMPLControlSolverInitializer() : InitializerBase(),
        Debug( false) ,
        MaxIterations( 100) ,
    ...
    OMPLControlSolverInitializer( std::string _Name, Eigen::VectorXd _StateLimits, bool _Debug =  false, int _MaxIterations =  100, double _MaxIterationTime =  10.0, double _ConvergenceTolerance =  1e-3, int _Seed =  -1, bool _ApproximateSolution =  true) : InitializerBase(),
        Name(_Name) ,
    ...
    OMPLControlSolverInitializer(const Initializer& other) : OMPLControlSolverInitializer()
    {
    ...
    {
        return (Initializer)OMPLControlSolverInitializer();
    }
    ...
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer OMPLControlSolverInitializer requires property Name to be set!");
        if(!other.HasProperty("StateLimits") || !other.properties_.at("StateLimits").IsSet()) ThrowPretty("Initializer OMPLControlSolverInitializer requires property StateLimits to be set!");
    }
