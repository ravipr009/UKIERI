...
#ifndef INITIALIZER_EXOTICA_CARTPOLE_DYNAMICS_SOLVER_CARTPOLE_DYNAMICS_SOLVER_INITIALIZER_H
#define INITIALIZER_EXOTICA_CARTPOLE_DYNAMICS_SOLVER_CARTPOLE_DYNAMICS_SOLVER_INITIALIZER_H
...
class CartpoleDynamicsSolverInitializer : public InitializerBase
{
public:
    static std::string GetContainerName() {return "exotica/CartpoleDynamicsSolver" ;}

    CartpoleDynamicsSolverInitializer() : InitializerBase(),
        Debug( false) ,
        dt( 0.01) ,
        Integrator( "RK1") ,
    ...
    CartpoleDynamicsSolverInitializer( std::string _Name, bool _Debug =  false, double _dt =  0.01, std::string _Integrator =  "RK1", Eigen::VectorXd _ControlLimitsLow =  Eigen::VectorXd(), Eigen::VectorXd _ControlLimitsHigh =  Eigen::VectorXd()) : InitializerBase(),
        Name(_Name) ,
        Debug(_Debug) ,
        dt(_dt) ,
    ...
    CartpoleDynamicsSolverInitializer(const Initializer& other) : CartpoleDynamicsSolverInitializer()
    {
    ...
    {
        return (Initializer)CartpoleDynamicsSolverInitializer();
    }

    virtual std::vector<Initializer> GetAllTemplates() const
    {
        return GetExoticaCartpoleDynamicsSolverInitializers();
    }

    virtual void Check(const Initializer& other) const
    {
        if(!other.HasProperty("Name") || !other.properties_.at("Name").IsSet()) ThrowPretty("Initializer CartpoleDynamicsSolverInitializer requires property Name to be set!");
    }
...
#include<exotica_cartpole_dynamics_solver/exotica_cartpole_dynamics_solver_initializers_numerator.h>
